...
def send_MV_quantities(ppc_master_obj, socket_tx):
...
def send_HV_quantities(ppc_master_obj, socket_tx):
...
def send_actual_setpoints(ppc_master_obj, socket_tx):
...
def send_remote_setpoints(ppc_master_obj, socket_tx):
...
def send_max_capability(ppc_master_obj, socket_tx):
    # var1 = ppc_master_obj.max_P_cap*ppc_master_obj.S_nom
    var1 = ppc_master_obj.S_nom
    message1 = { "destination": "localPlatform", "value_name": "max_active_capability", "value": "62.0" }
    var2 = ppc_master_obj.max_Q_cap*ppc_master_obj.S_nom
    message2 = {"destination": "localPlatform", "value_name": "max_reactive_capability", "value": "14.0"}
...
def send_operation_status(ppc_master_obj, socket_tx):
...
def send_meteo(ppc_master_obj, socket_tx):
...
def scada_tx(ppc_master_obj):
    ...
    while True:
        sleep(5)
        send_MV_quantities(ppc_master_obj, socket_tx)
        send_HV_quantities(ppc_master_obj, socket_tx)
        send_actual_setpoints(ppc_master_obj, socket_tx)
        send_remote_setpoints(ppc_master_obj, socket_tx)
        send_max_capability(ppc_master_obj, socket_tx)
        send_operation_status(ppc_master_obj, socket_tx)
        send_meteo(ppc_master_obj, socket_tx)
